 ...
 *
 * @uses blogmax_header_style()
 */
function blogmax_custom_header_setup() {
	add_theme_support( 'custom-header', 
		apply_filters( 'blogmax_custom_header_args', 
			array(
				...
				'height'                 => 528,
				'flex-height'            => true,
				'wp-head-callback'       => 'blogmax_header_style',
			)
		)
	);
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'blogmax' ),
		),
	) );
}
add_action( 'after_setup_theme', 'blogmax_custom_header_setup' );

if ( ! function_exists( 'blogmax_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see blogmax_custom_header_setup().
	 */
	function blogmax_header_style() {
		...
		wp_add_inline_style( 'blogmax-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'blogmax_header_style', 10 );